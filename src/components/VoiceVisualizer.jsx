...
function syntheticLevels(frame) {
  const levels = new Array(BAR_COUNT);
  const t = frame * 0.05;
  for (let i = 0; i < BAR_COUNT; i += 1) {
    const wave =
...
const VoiceVisualizer = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (syntheticVisualizer) {
        frameRef.current += 1;
        drawBars(ctx, width, height, syntheticLevels(frameRef.current));
    ...
    if (syntheticVisualizer && isActive && status === 'listening') {
      rafId = window.requestAnimationFrame(render);
    ...
    render();
    return undefined;
  }, [frequencyData, isActive, status, syntheticVisualizer]);
...
VoiceVisualizer.propTypes = {
  ...
  syntheticVisualizer: PropTypes.bool,
};
